namespace adventOfCode7b {
  ...
  function getCardsValue(cards: string) {
    ...
    if (
      fiveOfKinds === 1 ||
      (fourOfKinds === 1 && numberOfJokers === 1) ||
      (threeOfKinds === 1 && numberOfJokers === 2) ||
      (pairs === 1 && numberOfJokers === 3) ||
      numberOfJokers >= 4
    )
      return 6;
    if (
      fourOfKinds === 1 ||
      (threeOfKinds === 1 && numberOfJokers === 1) ||
      (pairs === 1 && numberOfJokers === 2) ||
      numberOfJokers === 3
    )
      return 5;
    if (
      (threeOfKinds === 1 && pairs === 1) ||
      (pairs === 2 && numberOfJokers === 1)
    )
      return 4;
    if (
      threeOfKinds === 1 ||
      (pairs === 1 && numberOfJokers === 1) ||
      numberOfJokers === 2
    )
      return 3;
    if (pairs === 2) return 2;
    if (pairs === 1 || numberOfJokers === 1) return 1;
    return 0;
  }
